...
class RadarApp:
    def __init__(self, root: tk.Tk, journal_dir: str | None = None) -> None:
        ...
        # Body table
        cols = ("body", "type", "dist", "bio", "value", "firsts")
        self.tree = ttk.Treeview(root, columns=cols, show="headings", selectmode="browse")
        for cid, txt, w, anchor in (
            ("body", "Body", 200, "w"), ("type", "Type", 140, "w"),
            ("dist", "Dist (ls)", 85, "e"), ("bio", "Bio", 44, "e"),
            ("value", "Value", 80, "e"), ("firsts", "Firsts available", 210, "w"),
        ):
    ...
    def _refresh(self) -> None:
        ...
        worth = r.firsts_value()
        worth_txt = f"  —  ~{_cr(worth)} cr in firsts" if worth else ""
        if r.undiscovered_system:
            self.banner.configure(text=f"★ {r.system or '?'}  —  UNDISCOVERED SYSTEM{worth_txt}", fg=GREEN)
        elif avail:
            self.banner.configure(text=f"{r.system or '?'}  —  {avail} bodies with a first{worth_txt}", fg=GOLD)
        else:
        ...
        for b in r.rows():
            ...
            self.tree.insert("", "end", tags=(tag,), values=(
                b.name or "?", b.kind or "?", dist, b.bio_signals or "", _cr(b.value()), "  ".join(badges) or "-",
            ))
